...
class LSTMLayer(lasagne.layers.LSTMLayer):
    ...
    def __init__(self, incoming, num_units,
        ...
        if mode=='binary' or mode=='ternary' or mode=='dual-copy' or mode=='quantize':
            super(LSTMLayer, self).__init__(incoming, num_units, peepholes = False, **kwargs) # ML: claim peepholes to be False?
        ...
        self.high = np.float32(np.sqrt(6. / (num_inputs + num_units)))  # ML: 6 is important?
    ...
    def get_output_for(self, inputs,deterministic=False, **kwargs):
        ...
        if self.round_hid:
            self.Wb_hid_to_ingate = apply_regularization(self.W_hid_to_ingate,hid=True)
            self.Wb_hid_to_forgetgate = apply_regularization(self.W_hid_to_forgetgate,hid=True)
            self.Wb_hid_to_cell = apply_regularization(self.W_hid_to_cell,hid=True)
            self.Wb_hid_to_outgate = apply_regularization(self.W_hid_to_outgate,hid=True)
        ...
        if self.round_input_weights:
            ...
            self.W_in_to_outgate = self.Wb_hid_to_outgate
