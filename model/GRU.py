...
class Gate(lasagne.layers.Gate):
    ...
    def __init__(self,mode='normal',H=1.0,nonlinearity=lasagne.nonlinearities.sigmoid,bias_init=lasagne.init.Constant(-1.), **kwargs):
        ...
        # ML: it will not be used since we only constrain the weight to limited precision format
...
class GRULayer(lasagne.layers.GRULayer):
    ...
    def __init__(self, incoming, num_units,
        ...
        # ML: refer to BNN, the dummy variables is useless?
    ...
    def get_output_for(self, inputs,deterministic=False, **kwargs):
        ...
        if self.round_bias:
            br_updategate = self.b_updategate
            br_resetgate = self.b_resetgate
            br_hidden_update = self.b_hidden_update
        ...
        input = inputs[0]    # ML: the symbolic representation should be the only one. like BNN, mlk.
        ...
        if self.round_bias:
            self.b_updategate = br_updategate
            self.b_resetgate = br_resetgate
            self.b_hidden_update = br_hidden_update
...
def compute_rnn_grads(loss,network):
    ...
    for layer in layers:
        ...
        if params:
            for param in params:
                #print(param.name)
                if param.name == layer.name + '.'+ 'W_in_to_updategate':
                    grads.append(theano.grad(loss, wrt=layer.Wb_in_to_updategate))
                elif param.name == layer.name + '.'+ 'W_in_to_resetgate':
                    grads.append(theano.grad(loss, wrt=layer.Wb_in_to_resetgate))
                elif param.name == layer.name + '.'+ 'W_in_to_hidden_update':
                    grads.append(theano.grad(loss, wrt=layer.Wb_in_to_hidden_update))
                elif param.name == layer.name + '.'+ 'W_hid_to_updategate':
                    grads.append(theano.grad(loss, wrt=layer.Wb_hid_to_updategate))
                elif param.name == layer.name + '.'+ 'W_hid_to_resetgate':
                    grads.append(theano.grad(loss, wrt=layer.Wb_hid_to_resetgate))
                elif param.name == layer.name + '.'+ 'W_hid_to_hidden_update':
                    grads.append(theano.grad(loss, wrt=layer.Wb_hid_to_hidden_update))
                elif param.name == layer.name + '.'+ 'b_updategate':
                    grads.append(theano.grad(loss, wrt=layer.bb_updategate))
                elif param.name == layer.name + '.'+ 'b_resetgate':
                    grads.append(theano.grad(loss, wrt=layer.bb_resetgate))
                elif param.name == layer.name + '.'+ 'b_hidden_update':
                    grads.append(theano.grad(loss, wrt=layer.bb_hidden_update))
